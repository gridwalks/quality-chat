...
export default async (req, context) => {
  ...
  try {
    ...
    if (!identity || !clientContext || !clientContext.identity || !clientContext.identity.token) {
      return new Response(JSON.stringify({ error: "Missing Identity admin token" }), { status: 401, headers: { "Content-Type":"application/json", ...CORS } });
    }
    const headers = { Authorization: `Bearer ${clientContext.identity.token}`, "Content-Type": "application/json" };
    let url = "";
    if (action === "invite") url = `${identity.url}/invite`;
    else if (action === "recovery") url = `${identity.url}/recover`;
    else return new Response(JSON.stringify({ error: "Unknown action" }), { status: 400, headers: { "Content-Type":"application/json", ...CORS } });
